...
def _aliases_bin_dirs() -> list[str]:
    """Candidate directories holding the `aliases` console script, best first.

    ~/.local/bin is always included: `aliases setup` may be run from a dev
    checkout, and baking only that venv into ~/.bash_aliases would break the
    shell once the checkout moves.
    """
    dirs: list[str] = []
    found = shutil.which("aliases")
    if found:
...
def _generate_bash_aliases(config_dir: Path) -> str:
    bin_dirs = " ".join(f'"{d}"' for d in _aliases_bin_dirs())
    return f"""\
...
# ── PATH ────────────────────────────────────────────────────────────────────
# Stock ~/.bashrc sources this file *before* it adds ~/.local/bin to PATH, so
# every `aliases` call below would silently fall back to its defaults: eza would
# ignore eza.icons and the prompt would stay on its plain fallback until the
# first cd. Prepend the install dir here; the later .bashrc line is then a no-op.
for _ali_bin in {bin_dirs}; do
    [ -d "$_ali_bin" ] || continue
    case ":${{PATH}}:" in
        *":$_ali_bin:"*) ;;
        *) PATH="$_ali_bin:${{PATH}}" ;;
