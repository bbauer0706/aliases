...
TodoTUI::TodoTUI(TodoManager* todo_manager) 
...
void TodoTUI::draw_todo_list() {
    ...
    // Draw todos
    for (int i = 0; i < list_height && (i + state_.scroll_offset) < static_cast<int>(state_.filtered_todos.size()); i++) {
        ...
        if (total_length > max_x - 1) {
            int available_for_main = max_x - 1 - status_str.length() - priority_part.length() - category_part.length() - 3; // -3 for "..."
...
void TodoTUI::update_filtered_todos() {
    ...
    if (state_.sort_mode == 0) {
        ...
        std::sort(state_.filtered_todos.begin(), state_.filtered_todos.end(), 
...
void TodoTUI::move_selection(int delta) {
    ...
    state_.current_selection = std::max(0, std::min(
        static_cast<int>(state_.filtered_todos.size()) - 1,
        state_.current_selection + delta
    ));
...
void TodoTUI::toggle_todo_completion() {
    if (state_.filtered_todos.empty() || 
        state_.current_selection >= static_cast<int>(state_.filtered_todos.size())) {
        return;
    }
...
void TodoTUI::uncomplete_todo(int id) {
...
int TodoTUI::get_priority_color(int priority) const {
    switch (priority) {
        case 3: return 2; // Red for high priority
        case 2: return 3; // Yellow for medium priority  
        case 1: return 4; // Blue for low priority
        default: return 0; // No color for no priority
    }
}

std::string TodoTUI::get_status_string(bool completed) const {
    return completed ? "x" : " ";
}
...
void TodoTUI::exit_category_filter() {
    state_.in_category_filter_mode = false;
}
...
void TodoTUI::draw_category_filter() {
    ...
    for (int i = 0; i < list_height && (i + state_.category_scroll_offset) < static_cast<int>(state_.available_categories.size()); i++) {
        ...
        bool is_selected = std::find(state_.selected_categories.begin(), 
                                   state_.selected_categories.end(), category) != state_.selected_categories.end();
...
void TodoTUI::move_category_selection(int delta) {
    ...
    state_.category_selection = std::max(0, std::min(
        static_cast<int>(state_.available_categories.size()) - 1,
        state_.category_selection + delta
    ));
...
void TodoTUI::toggle_category_selection() {
    if (state_.category_selection >= 0 && state_.category_selection < static_cast<int>(state_.available_categories.size())) {
