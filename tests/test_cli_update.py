"""Tests for aliases-cli update command."""
...
class TestUpdateCheck:
    def test_already_up_to_date(self, runner):
        with patch("aliases_cli.commands.update_cmd._fetch_latest_tag", return_value="v2.1.1"), \
             patch("aliases_cli.commands.update_cmd.__version__", "2.1.1"):
    ...
    def test_update_available_triggers_install(self, runner):
        ...
        with patch("aliases_cli.commands.update_cmd._fetch_latest_tag", return_value="v9.9.9"), \
             patch("aliases_cli.commands.update_cmd.__version__", "2.1.1"), \
    ...
    def test_github_unreachable_exits_1(self, runner):
        with patch("aliases_cli.commands.update_cmd._fetch_latest_tag", return_value=None):
            result = runner.invoke(cli, ["update"])
        assert result.exit_code == 1
...
class TestFetchLatestTag:
    def test_returns_first_v_tag(self):
        from aliases_cli.commands.update_cmd import _fetch_latest_tag
