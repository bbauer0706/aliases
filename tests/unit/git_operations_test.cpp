...
TEST_F(GitOperationsTest, IsGitRepositoryTrue) { EXPECT_TRUE(GitOperations::is_git_repository(test_repo_.string())); }
...
TEST_F(GitOperationsTest, CheckoutBranchSuccess) {
    ...
    std::system(
        ("cd " + test_repo_.string() + " && git checkout master >/dev/null 2>&1 || git checkout main >/dev/null 2>&1")
            .c_str());
...
TEST_F(GitOperationsTest, IsMainBranchMain) { EXPECT_TRUE(GitOperations::is_main_branch("main")); }

TEST_F(GitOperationsTest, IsMainBranchMaster) { EXPECT_TRUE(GitOperations::is_main_branch("master")); }

TEST_F(GitOperationsTest, IsMainBranchFeatureBranch) { EXPECT_FALSE(GitOperations::is_main_branch("feature-branch")); }

TEST_F(GitOperationsTest, IsMainBranchEmptyString) { EXPECT_FALSE(GitOperations::is_main_branch("")); }
