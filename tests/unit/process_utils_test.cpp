...
TEST_F(ProcessUtilsTest, CommandExistsEmptyString) { EXPECT_FALSE(ProcessUtils::command_exists("")); }
...
TEST_F(ProcessUtilsTest, ExecuteCommandWithRedirection) {
    ...
    auto result = ProcessUtils::execute("echo 'redirected' > " + test_file.string());
